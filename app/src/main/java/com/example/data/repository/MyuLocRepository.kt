...
class MyuLocRepository(private val database: MyuLocDatabase) {
    ...
    suspend fun addOfflineTracks(tracks: List<OfflineTrack>) {
        tracks.chunked(250).forEach { chunk ->
            offlineTrackDao.insertOfflineTracks(chunk)
        }
    }
    ...
    suspend fun getPreferencesWithPrefix(prefix: String): List<PreferenceEntity> {
        return preferenceDao.getPreferencesWithPrefix(prefix)
    }
